...
{{- define "lib.sh" }}
    ...
    # shellcheck disable=SC2034  # is_master is consumed by scripts that source lib.sh
    get_redis_role() {
      is_master=$(
        redis-cli \
        {{- if .Values.auth }}
...
{{- define "trigger-failover-if-master.sh" }}
    ...
    if [ "$is_master" -eq 1 ]; then
      ...
      # Give Sentinel a moment to publish the master change to clients via pub/sub.
      sleep 2

      # Disconnect Sentinel clients so they reconnect to the new master immediately.
      redis-cli \
      {{- if .Values.sentinel.auth }}
          -a "${SENTINELAUTH}" --no-auth-warning \
      {{- end }}
...
{{- define "redis-prestop.sh" }}
    ...
    while [ "$is_master" -eq 1 ] && [ "$timeout" -gt 0 ]; do
      echo "[$0] This Redis node is currently master, waiting for failover to flip the role (${timeout}s)."
      sleep 1
      get_redis_role
      timeout=$((timeout - 1))
    done
